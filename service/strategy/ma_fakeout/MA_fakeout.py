...
import service.strategy.ma_fakeout.MA_fakeout_BL as StrategyBL
...
class StrategyService:
    ...
    def initiatePositionSignal(self, candleData: list[ExchangeDataModel.CandleStickType], marketDepth=None, backTestingEnabled=False) -> ExchangeDataModel.InitiatePositionData:
        initiateSignal, positionType, stoploss = StrategyBL.getTradeEntry(
        ...
        if initiateSignal != StrategyUtils.TradeEntryConfidenceScoreType.NO_TRADE:
            if not backTestingEnabled:
                return ExchangeDataModel.InitiatePositionData(True, candleData[len(candleData) - 1].close, "market", positionType, stoploss)
            else:
                return ExchangeDataModel.InitiatePositionData(True, candleData[len(candleData) - 1].open, "market", positionType, stoploss)
